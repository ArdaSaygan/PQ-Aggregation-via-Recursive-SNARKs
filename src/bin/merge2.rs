...
use pq_aggregation_methods_2::PQ_AGGREGATION_GUEST_2_ID;
...
pub fn main() {
    ...
    let start = Instant::now();
    let acc00 =
        pq_aggregation::aggregate_signatures_2(sig_1, pub_key_1, 1, message, &public_keys, image_id);
    ...
    let start = Instant::now();
    let acc01 =
        pq_aggregation::aggregate_signatures_2(sig_2, pub_key_2, 2, message, &public_keys, image_id);
    ...
    // let start = Instant::now();
    let acc10 = pq_aggregation::merge_accumulators_2(
        acc_1, acc_2, 
        image_id,
    );
    ...
    log_message(
        &mut log_file,
        &format!(
            "Merged acc00 + acc01 = acc10, {} seconds",
