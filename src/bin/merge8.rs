...
use pq_aggregation_methods_8::PQ_AGGREGATION_GUEST_8_ID;
...
pub fn main() {
    ...
    let epoch: u32 = 2;
    let image_id: [u32; 8] = PQ_AGGREGATION_GUEST_8_ID;
    let message: [u8; 32] = {
    ...
    let start = Instant::now();
    let acc00 =
        pq_aggregation::aggregate_signatures_8(sig_1, pub_key_1, 1, message, &public_keys, image_id);
    ...
    let start = Instant::now();
    let acc01 =
        pq_aggregation::aggregate_signatures_8(sig_2, pub_key_2, 2, message, &public_keys, image_id);
    ...
    let start = Instant::now();
    let acc10 = pq_aggregation::merge_accumulators_8(
    ...
    log_message(
        &mut log_file,
        &format!(
            "Merged acc00 + acc01 + acc00 + acc01 + acc00 + acc01 + acc00 + acc01 = acc10, {} seconds",
