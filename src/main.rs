...
pub fn main() {
    ...
    let start = Instant::now();
    let acc00 =
        pq_aggregation::aggregate_signatures(sig_1, pub_key_1, 1, message, &public_keys, image_id);
    ...
    let start = Instant::now();
    let acc01 =
        pq_aggregation::aggregate_signatures(sig_2, pub_key_2, 2, message, &public_keys, image_id);
    ...
    let start = Instant::now();
    let acc10 = pq_aggregation::merge_accumulators(
